pipeline {
  ...
  stages {
    ...
    stage('deploy kubernetes') {
      steps {
        container('kustomize') {
          sh "sed -i.bak 's#set_image#$asia-northeast3-docker.pkg.dev/phonic-realm-360311/test-img-registry/quickstart-image:${BRANCH_NAME}_${IMAGE_TAG}#' ./deployment.yaml"
